...
try {
	...
	String Query2 = "SELECT num, title, id, del, date, text, categorize, filename FROM recipe WHERE num=?";
	...
	while(rs1.next()){
		...
		int del = rs1.getInt("del");
		String date = rs1.getString("date");
		String categorize = rs1.getString("categorize");
...
<td width=100><%=del%></td>
<td width=40><input type="button" value="맛있어요" onClick="javascript:location.replace('like.jsp?rno=<%=rno%>')"></td>
</tr>
...
if(filename == null) {
	...
	String IMGURL="../image";
%>
<img src = "../image/<%=filename%>" style='width:400px;height:300px'>
<%
}
%>
</td>
...
<input type="button" value="목록" onClick="javascript:location.replace('BoardList.jsp?column=<%=column %>&key=<%=encoded_key %>')">
</td>

<td width="310" align=right>
<input type="button" value="수정" onClick="javascript:location.replace('BoardModify.jsp?rno=<%=rno%>&column=<%=column %>&key=<%=encoded_key %>')" style=cursor:hand>&nbsp;&nbsp;

<input type="button" value="삭제"  onClick="javascript:location.replace('BoardDeleteProc.jsp?rno=<%=rno %>&column=<%=column %>&key=<%=encoded_key%>')" style=cursor:hand>
</td>
</tr>
</table>
<%
